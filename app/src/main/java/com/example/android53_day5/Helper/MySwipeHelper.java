        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if (dX < 0) {
                ...
                if (!buttonBuffer.containsKey(pos)) {
                    instantiateMyButton(viewHolder, buttons);
                    buttonBuffer.put(pos, buttons);
                } else {
                    buttons = buttonBuffer.get(pos);
                }
                translationX = dX * buttons.size() * buttonWidth / itemView.getWidth();
                drawButton(c, itemView, buttons, pos, translationX);
            }
        }
        super.onChildDraw(c, recyclerView, viewHolder, translationX, dY, actionState, isCurrentlyActive);
    }

    private void drawButton(Canvas c, View itemView, List<MyButton> buttons, int pos, float translationX) {
        float right = itemView.getRight();
        float dButtonWidth = -1 * translationX / buttons.size();
        for (MyButton myButton : buttons) {
            float left = right - dButtonWidth;
            myButton.onDraw(c, new RectF(left, itemView.getTop(), right, itemView.getBottom()), pos);
            right = left;
        }
    }

    public abstract void instantiateMyButton(RecyclerView.ViewHolder viewHolder, List<MyButton> buttons);
}
